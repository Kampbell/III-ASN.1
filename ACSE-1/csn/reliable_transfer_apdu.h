...
using Remote_Operations_Information_Objects::CONTRACT;
extern const ASN1::OBJECT_IDENTIFIER rTSE_abstract_syntax;
...
public:
    ...
    void swap(RTSE_apdus& that) { Inherited::swap(that); }

    RTSE_apdus * clone() const;
    static bool equal_type(const ASN1::AbstractData& type);
    static const InfoType theInfo;
private:
    static const void* selectionInfos[6];
    static unsigned selectionTags[6];
    static const char* selectionNames[6];
}; // end class RTSE_apdus
...
public:
    ...
    public:
        rTSE();
        const ASN1::OBJECT_IDENTIFIER id;
        CONTRACT::value_type make() const
        { return CONTRACT::value_type(id,&info ); }
    private:
        typedef CONTRACT::info_type Info;
        Info info;
    }; // class rTSE
    const rTSE& get_rTSE() const
    {  return m_rTSE; }
private:
    rTSE m_rTSE;
}; // class Module

#include "reliable_transfer_apdu.inl"

} // namespace Reliable_Transfer_APDU

#endif // __RELIABLE_TRANSFER_APDU_H

// end of reliable_transfer_apdu.h
