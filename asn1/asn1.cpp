...
ENUMERATED::ENUMERATED(int value, const void* info)
	: AbstractData(info), value(value) {
	assert(getMaximum() >= 0);
}
...
void RELATIVE_OID::encodeCommon(octets & encodecObjectId) const {
	...
	ASN1_STD insert_iterator<octets > insertItr(encodecObjectId, encodecObjectId.begin());
	while (--length > 0) {
		if (subId < 128)
...
OBJECT_IDENTIFIER::OBJECT_IDENTIFIER(const initializer_list<int>& list) : AbstractData(&theInfo) {
	value.resize(list.size(), 0);
	initializer_list<int>::iterator it;
	for (it = list.begin(); it != list.end(); ++it)
		value[it - list.begin()] = *it;
}

OBJECT_IDENTIFIER& OBJECT_IDENTIFIER::assign(unsigned nelem, ... /*list of unsigned*/) {
	va_list lst;
	va_start(lst, nelem);
	assign(nelem, lst);
	va_end(lst);
...
OBJECT_IDENTIFIER& OBJECT_IDENTIFIER::assign(unsigned nelem, va_list lst) {
	value.resize(nelem, 0);
	for(unsigned i=0; i<nelem; ++i)
		value[i] = va_arg(lst, unsigned);
	return *this;
...
OBJECT_IDENTIFIER& OBJECT_IDENTIFIER::assign(const initializer_list<int>& list) {
	value.resize(list.size(), 0);
	initializer_list<int>::iterator it;
	for (it = list.begin(); it != list.end(); ++it)
		value[it - list.begin()] = *it;
...
void OBJECT_IDENTIFIER::encodeCommon(octets & encodecObjectId) const {
	unsigned length = value.size();
	if (length < 2) {
		// Thise case is really illegal, but we have to do SOMETHING
		encodecObjectId.resize(0);
...
BIT_STRING::BIT_STRING() : ConstrainedObject(&theInfo) {
}
