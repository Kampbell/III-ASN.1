...
bool CoderTest(const char* str, CoderEnv& env, const AbstractData& v1, AbstractData& v2, const char* encodedStrm, int lineno) {
	bool result = true;
	vector<char> strm;
	if (!encode(v1, &env, back_inserter(strm)) && equal(strm.begin(), strm.end(), encodedStrm)) {
...
ASN1Test::ASN1Test(const string& name) : CppUnit::TestCase(name) {
}
ASN1Test::~ASN1Test() {
}

#ifdef ASN1_HAS_IOSTREAM
void ASN1Test::testValueNotationTests() {
	CoderEnv env;
...
void ASN1Test::testPERCoder() {
	CoderEnv env;
	env.set_encodingRule(CoderEnv::per_Basic_Aligned);

	cout << endl;
	INTEGER i1(3), i2;
	TEST("value INTEGER ::= 3 (PER)", env ,i1, i2,"\x01\x03");

	i1 = -3;
	TEST("value INTEGER ::= -3 (PER)", env ,i1, i2,"\x01\xfd");

	i1 = 127;
	TEST("value INTEGER ::= 127 (PER)", env ,i1, i2,"\x01\x7f");

	i1 = -128;
	TEST("value INTEGER ::= -128 (PER)", env ,i1, i2,"\x01\x80");

	i1 = 128;
	TEST("value INTEGER ::= 128 (PER)", env ,i1, i2,"\x02\x00\x80");

	i1 = 256;
	TEST("value INTEGER ::= 256 (PER)", env ,i1, i2,"\x02\x01\x00");

	ASN1::Constrained_INTEGER<FixedConstraint, 3, 6> i3 = 3, i4;
	TEST("value INTEGER(3..6) ::= 3 (PER)", env ,i3, i4,"\x00");

	ASN1::Constrained_INTEGER<FixedConstraint, 4000, 4255> i5 = 4002, i6;
	TEST("value INTEGER(4000..4255) ::= 4002 (PER)", env ,i5, i6,"\x02");

	ASN1::Constrained_INTEGER<ExtendableConstraint, 3, 6> i7 = 5, i8;
	TEST("value INTEGER(3..6,...) ::= 5 (PER)", env ,i7, i8,"\x40");

	i7 = 8;
	TEST("value INTEGER(3..6,...) ::= 8 (PER)", env ,i7, i8,"\x80\x01\x08");

	ASN1::Constrained_INTEGER<ExtendableConstraint, 1, 65538> i9 = 1, i10;
	TEST("value INTEGER(1..65538,...) ::= 1 (PER)", env ,i9, i10,"\x00\x00");

	i9 = 257;
	TEST("value INTEGER(1..65538,...) ::= 257 (PER)", env ,i9, i10,"\x20\x01\x00");

	i9 = 65538;
	TEST("value INTEGER(1..65538,...) ::= 65538 (PER)", env ,i9, i10,"\x40\x01\x00\x01");

	i9 = 65539;
	TEST("value INTEGER(1..65538,...) ::= 65539 (PER)", env ,i9, i10,"\x80\x03\x01\x00\x03");

	ASN1::NumericString ns1("12345"), ns2;
	TEST("value NumericString ::= \"12345\" (PER)", env ,ns1, ns2,"\x05\x23\x45\x60");

	using namespace MyHTTP;

	IA5Str01  ia501str1("012345"), ia501str2;
	TEST("value IA5String (SIZE (6)) ::= \"012345\" (PER)",
		 env ,ia501str1, ia501str2,"\x30\x31\x32\x33\x34\x35");

	IA5Str02  ia502str1("0123456"), ia502str2;
	TEST("value IA5String (SIZE (5..20)) ::= \"0123456\" (PER)",
		 env ,ia502str1, ia502str2,"\x20\x30\x31\x32\x33\x34\x35\x36");

	IA5Str03  ia503str1("abc"), ia503str2;
	TEST("value IA5String (SIZE (0..8)) ::= \"abc\" (PER)",
		 env ,ia503str1, ia503str2,"\x30\x61\x62\x63");

	IA5Str04  ia504str1("ABCDEFGH"), ia504str2;
	TEST("value IA5String ::= \"ABCDEFGH\" (PER)",
		 env ,ia504str1, ia504str2,"\x08\x41\x42\x43\x44\x45\x46\x47\x48");

	IA5Str05  ia505str1("abc"), ia505str2, ia505str3("abcdefgh");
	TEST("value IA5String (SIZE (0..7, ..., 8)) ::= \"abc\" (PER)",
		 env ,ia505str1, ia505str2,"\x30\x61\x62\x63");
	/*
	  TEST("value IA5String (SIZE (0..7, ..., 8)) ::= \"abcdefgh\" (PER)",
	    env ,ia505str3, ia505str2,"\x08\x61\x62\x63\x64\x65\x66\x67\x68");
	*/
	IA5Str08 ia508str1("*2601#"), ia508str2;
	TEST("value IA5String (FROM (\"0123456789#*,\")) ::= \"*2601#\" (PER)",
		 env ,ia508str1, ia508str2,"\x06\x15\x93\x40");

	IA5Str09 ia509str1("*2601#"), ia509str2;
	TEST("value IA5String (FROM (\"0123456789#*,ABCD\")) ::= \"*2601#\" (PER)",
		 env ,ia509str1, ia509str2,"\x06\x2a\x32\x36\x30\x31\x23");

	IA5Str10 ia510str1("TAG"), ia510str2;
	TEST("value IA5String (FROM (\"ACGT\")^SIZE(3)) ::= \"TAG\" (PER)",
		 env ,ia510str1, ia510str2,"\xC8");

	IA5Str11 ia511str1("TFFT"), ia511str2;
	TEST("value IA5String (FROM (\"TF\")) (SIZE(0..16)) ::= \"TFFT\" (PER)",
...
void ASN1Test::testBERCoder() {
	...
	TEST("value Choice4 ::= f2 : f4 : 5" , env, c7, c8 , "\x02\x01\x05");

	S4 s4_1, s4_2;
	s4_1.set_name("A");
	s4_1.set_attrib().select_f2().select_f3(true);
	TEST("value S4 ::= { name A, attrib : f2 : f3 TRUE }", env, s4_1, s4_2, "\x30\x06\x1A\x01\x41\x01\x01\xFF");
...
void ASN1Test::testABSTRACT_SYNTAX() {
/*
...
*/
	// my-abstract-syntax  ABSTRACT-SYNTAX ::=  { INTEGER IDENTIFIED BY {joint-iso-itu-t 23 42 9 43 1} }
	{
		ABSTRACT_SYNTAX::info_type it = &INTEGER::theInfo;
		ABSTRACT_SYNTAX::key_type kt{2, 23, 42, 9, 43, 1};
		ABSTRACT_SYNTAX::mapped_type mt = &it;
		ABSTRACT_SYNTAX::value_type vt(kt, mt);
		abstract_syntaxes.insert(vt);
	}
	{
		ABSTRACT_SYNTAX::key_type kt{2, 23, 42, 9, 43, 1};
		ABSTRACT_SYNTAX::const_iterator cit;
		cit = abstract_syntaxes.find(kt);
		if (cit != abstract_syntaxes.end()) {
			ASN1::AbstractData* ad = cit->second->get_Type();
			*static_cast<INTEGER*>(ad) = 3;
			clog << cit->first << ":" << typeid(*ad).name() << ' ' << ad->asValueNotation() << endl;
			ad->print_on(clog);
		}
	}
}
void ASN1Test::testTYPE_IDENTIFIER() {
/*
...
*/
	{
		TYPE_IDENTIFIER::info_type it = &INTEGER::theInfo;
		TYPE_IDENTIFIER::key_type kt{2, 23, 42, 9, 43, 1};
		TYPE_IDENTIFIER::mapped_type mt = &it;
		TYPE_IDENTIFIER::value_type vt(kt, mt);
		type_identifiers.insert(vt);
	}
	{
		TYPE_IDENTIFIER::key_type kt{2, 23, 42, 9, 43, 1};
		TYPE_IDENTIFIER::const_iterator cit;
		cit = type_identifiers.find(kt);
		if (cit != type_identifiers.end()) {
			ASN1::AbstractData* ad = cit->second->get_Type();
			*static_cast<INTEGER*>(ad) = 3;
			clog << cit->first << ":" << typeid(*ad).name() << ' ' << ad->asValueNotation() << endl;
			ad->print_on(clog);
...
//	}
	...
	const octet* data;
	{
		...
		EXTERNAL::direct_reference::value_type direct_reference{2, 2, 3, 3};

		external1.set_direct_reference() = direct_reference;
		external1.set_indirect_reference() = 1;
		
		const string descriptor("descriptor");
		external1.set_data_value_descriptor() = descriptor;

		EXTERNAL::encoding::value_type encoding;
		
		octets octet_aligned{'a', 'b', 'c'};
		encoding.select_octet_aligned(octet_aligned);

		external1.set_encoding(encoding);

		result = encode(external1, &env, back_inserter(strm));
		assert(result);
		data = strm.data();
		result = decode(data, data + strm.size(), &env, external2);
		assert(result);
		assert(external1 == external2);
		clog << external2.asValueNotation() << endl;

	}
	{
		EXTERNAL::indirect_reference indirect_reference;
	}
	{
		EXTERNAL::data_value_descriptor data_value_descriptor;
	}
}
void ASN1Test::setUp() {
	env.set_encodingRule(CoderEnv::ber);
